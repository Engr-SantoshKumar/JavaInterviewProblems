...
public class _Goo2_05_01_Find_Shortest_Path_from_Source_to_Destination {
    ...
    // some variables to track no of steps taken
    static int noOfTraveledCell =0;
    static int nos_of_cells_in_current_layer = 1;
    static int nos_of_cells_in_next_layer = 0;
    ...
    public static int findShortestPath(char[][] grid){
       ...
       while(!rowQ.isEmpty()){ // we can even use colQ because both wil always have same elements
           ...
           if(grid[curR][curC] == 'd'){
               return noOfTraveledCell;
           }
           exploreNeighbours(curR, curC, grid, visitedGrid);
           nos_of_cells_in_current_layer --;
           if(nos_of_cells_in_current_layer ==0){
               noOfTraveledCell++;
               nos_of_cells_in_current_layer = nos_of_cells_in_next_layer;
               nos_of_cells_in_next_layer =0;
           }
       }
        return -1;
    }
